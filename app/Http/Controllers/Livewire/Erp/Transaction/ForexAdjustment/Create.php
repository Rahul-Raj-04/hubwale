...
class Create extends Component
{
	...
	public function addEntry()
	{
        ...
        if ($validator->fails()) {
			...
			$this->reset('debit');
			$this->reset('credit');
	        if (end($this->account_antries)['balance_type'] == 'DB'){
	        	$this->enable_debit = 0;
	        	$this->enable_credit = 1;
	        	$this->balance_type = 'CR';
	        } elseif (end($this->account_antries)['balance_type'] == 'CR') {
	        	$this->enable_credit = 0;
	        	$this->enable_debit = 1;
	...
	public function deleteEntry($key)
	{
		...
		unset($this->account_antries[$key]);
        $this->dispatchBrowserEvent('entry-table');
	}
	...
	public function submit()
	{
		...
		foreach ($this->account_antries as $key => $account_antrie) {
		...
		if ( $credit != $debit){
		...
		} elseif (count($this->account_antries) == 0) {
            ...
            }while (count($forexAdjustments) > 0);

			foreach ($this->account_antries as $key => $account_antry) {
            ...
            <div>
                ...
                <table class="table table-bordered text-nowrap border-bottom entry-table">
                    ...
                    <tbody>
                        ...
                        @foreach ($account_antries as $key => $account_entry)
                            <tr>
                                <td>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-sm btn-outline-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" wire:click=deleteEntry({{$key}})>
