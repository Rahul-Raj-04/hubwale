...
namespace App\Http\Controllers\Livewire\Erp\Transaction\CnDnEntry\CnEntryWithOutStock;
...
class Edit extends Component
{
    ...
    public $cnEntryWithOutStocks = []; 

    public function mount($cnEntryWithOutStock)
    {
        $this->cnEntryWithOutStocks = CNDNEntry::where('group_id', $cnEntryWithOutStock->group_id)->get();
        foreach ($this->cnEntryWithOutStocks as $key => $cnEntryWithOutStock) {
            $this->account_entries[] = [
                'id' => $cnEntryWithOutStock->id,
                'sales_purchase_ac_id' => $cnEntryWithOutStock->sales_purchase_ac_id,
                'gst_commodity_id' => $cnEntryWithOutStock->gst_commodity_id,
                'assess_amt' => $cnEntryWithOutStock->assess_amt
            ];
        }
        $this->balance_type = $cnEntryWithOutStock->balance_type;
        $this->party_account_id = $cnEntryWithOutStock->party_account_id;
        $this->invoice_type = $cnEntryWithOutStock->invoice_type;
        $this->effect = $cnEntryWithOutStock->effect;
        $this->tax_bill_of_supply = $cnEntryWithOutStock->tax_bill_of_supply;
        $this->reason = $cnEntryWithOutStock->reason;
        $this->vou_date = $cnEntryWithOutStock->vou_date;
        $this->voucher_no = $cnEntryWithOutStock->voucher_no;
        $this->doc_no = $cnEntryWithOutStock->doc_no;
        $this->doc_date = $cnEntryWithOutStock->doc_date;
        $this->original_bill_no = $cnEntryWithOutStock->original_bill_no;
        $this->original_bill_date = $cnEntryWithOutStock->original_bill_date;
        $this->narration = $cnEntryWithOutStock->narration;
            ...
            foreach ($this->account_entries as $key => $account_entry) {
                if (!$account_entry['id']) {
                    CNDNEntry::create([
                        ...
                        'model_name' => Enum::CN_ENTRY_W_O_STOCK,
                        'group_id' => $this->cnEntryWithOutStocks[0]->group_id
                    ]);
                    ...
                    $cnEntryWithOutStock = CNDNEntry::find($account_entry['id']);
                    $cnEntryWithOutStock->balance_type = $this->balance_type;
                    $cnEntryWithOutStock->party_account_id = $this->party_account_id;
                    $cnEntryWithOutStock->invoice_type = $this->invoice_type;
                    $cnEntryWithOutStock->effect = $this->effect;
                    $cnEntryWithOutStock->tax_bill_of_supply = $this->tax_bill_of_supply;
                    $cnEntryWithOutStock->reason = $this->reason;
                    $cnEntryWithOutStock->vou_date = $this->vou_date;
                    $cnEntryWithOutStock->voucher_no = $this->voucher_no;
                    $cnEntryWithOutStock->doc_no = $this->doc_no;
                    $cnEntryWithOutStock->doc_date = $this->doc_date;
                    $cnEntryWithOutStock->original_bill_no = $this->original_bill_no;
                    $cnEntryWithOutStock->original_bill_date = $this->original_bill_date;
                    $cnEntryWithOutStock->save();
                }
            }
            toast('CN entry w/o stock updated successfully!', 'success');
