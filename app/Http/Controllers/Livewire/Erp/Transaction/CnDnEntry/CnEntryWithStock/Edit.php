...
namespace App\Http\Controllers\Livewire\Erp\Transaction\CnDnEntry\CnEntryWithStock;
...
class Edit extends Component
{
    ...
    public $cnEntryWithStocks = [];

    public function mount($cnEntryWithStock)
    {
        $this->cnEntryWithStocks = CNDNEntry::where('group_id', $cnEntryWithStock->group_id)->get();
        foreach ($this->cnEntryWithStocks as $key => $cnEntryWithStock) {
            $this->account_entries[] = [
                'id' => $cnEntryWithStock->id,
                'service_id' => $cnEntryWithStock->service_id,
                'qty' => $cnEntryWithStock->qty,
                'currency_master_id' => $cnEntryWithStock->currency_master_id,
                'rate' => $cnEntryWithStock->rate,
                'amount' => $cnEntryWithStock->amount,
            ];
        }
        $this->balance_type = $cnEntryWithStock->balance_type;
        $this->party_account_id = $cnEntryWithStock->party_account_id;
        $this->stock_effect = $cnEntryWithStock->stock_effect;
        $this->invoice_type = $cnEntryWithStock->invoice_type;
        $this->effect = $cnEntryWithStock->effect;
        $this->tax_bill_of_supply = $cnEntryWithStock->tax_bill_of_supply;
        $this->reason = $cnEntryWithStock->reason;
        $this->vou_date = $cnEntryWithStock->vou_date;
        $this->voucher_no = $cnEntryWithStock->voucher_no;
        $this->doc_no = $cnEntryWithStock->doc_no;
        $this->doc_date = $cnEntryWithStock->doc_date;
        $this->original_bill_no = $cnEntryWithStock->original_bill_no;
        $this->original_bill_date = $cnEntryWithStock->original_bill_date;
        $this->narration = $cnEntryWithStock->narration;
    ...
    public function deleteEntry($key)
    {
        if($this->account_entries[$key]['id']) {
            $cnEntryWithOutStock = CNDNEntry::find($this->account_entries[$key]['id']);
            ...
            foreach ($this->account_entries as $key => $account_entry) {
                if (!$account_entry['id']) {
                    CNDNEntry::create([
                        ...
                        'model_name' => Enum::CN_ENTRY_WITH_STOCK,
                        'group_id' => $this->cnEntryWithStocks[0]->group_id
                    ]);
                    ...
                    $cnEntryWithStock = CNDNEntry::find($account_entry['id']);
                    $cnEntryWithStock->balance_type = $this->balance_type;
                    $cnEntryWithStock->party_account_id = $this->party_account_id;
                    $cnEntryWithStock->stock_effect = $this->stock_effect;
                    $cnEntryWithStock->invoice_type = $this->invoice_type;
                    $cnEntryWithStock->effect = $this->effect;
                    $cnEntryWithStock->tax_bill_of_supply = $this->tax_bill_of_supply;
                    $cnEntryWithStock->reason = $this->reason;
                    $cnEntryWithStock->vou_date = $this->vou_date;
                    $cnEntryWithStock->voucher_no = $this->voucher_no;
                    $cnEntryWithStock->doc_no = $this->doc_no;
                    $cnEntryWithStock->doc_date = $this->doc_date;
                    $cnEntryWithStock->original_bill_no = $this->original_bill_no;
                    $cnEntryWithStock->original_bill_date = $this->original_bill_date;
                    $cnEntryWithStock->narration = $this->narration;
                    $cnEntryWithStock->save();
                }
            }
            toast('CN entry with stock updated successfully!', 'success');
                ...
                <form wire:submit.prevent="submit">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select class="form-control form-control-sm form-select" wire:model.defer="stock_effect">
