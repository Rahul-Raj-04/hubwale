            ...
            <div class="card">
                <div class="card-header p-3">
                    ...
                    <div class="col-auto ms-auto d-print-none pe-0">
                        <div class="btn-list">
                            <a href="{{ route('erp.account-transaction.cash-bank-entry.bank-payment.create') }}" class="btn btn-sm btn-primary d-none d-sm-inline-block me-0">
                                <i class="fas fa-plus me-1"></i>
                                Add Bank Payment
                            </a>

                            <a href="{{ route('erp.account-transaction.cash-bank-entry.bank-payment.create') }}" class="btn btn-sm btn-primary d-sm-none me-0">
                        ...
                        <table class="table table-bordered text-nowrap border-bottom datatable-common">
                            ...
                            <tbody>
                                @foreach ($bankPayments as $key => $bankPayment)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{  date('d-m-Y', strtotime($bankPayment->date)) }}</td>
                                        <td>{{ $bankPayment->account->name }}</td>
                                        <td>
                                            <label class="custom-control custom-checkbox">
                                                <input id="{{'is_audit_'.$bankPayment->id}}" type="checkbox" onchange="updateIsAudit({{ $bankPayment->id }})" class="custom-control-input" value="{{$bankPayment->id}}" {{ $bankPayment->is_audit == 1 ? 'checked' : ''}}>
                                                <span class="custom-control-label"></span>
                                            </label> 
                                        </td>
                                        <td>{{ $bankPayment->reference_no }}</td>
                                        <td>{{ $bankPayment->opposite_account->name }}</td>
                                        <td>{{ $bankPayment->amount }}</td>
                                        <td>    
                                            <div class="{{ $bankPayment->is_audit == 1 ? '' : 'd-none'}}" id="{{'not-edited-'.$bankPayment->id}}">
                                            ...
                                            <div class="{{ $bankPayment->is_audit == 1 ? 'd-none' : ''}}" id="{{'edited-'.$bankPayment->id}}">
                                                <a href="{{route('erp.account-transaction.cash-bank-entry.bank-payment.edit', $bankPayment->id)}}" class="btn btn-sm btn-outline-info">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </span>
                                                </a>
                                                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete-modal-{{ $bankPayment->id }}">
                            ...
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.account-transaction.cash-bank-entry.bank-payment.destroy', $bankPayment->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('erp.account-transaction.cash-bank-entry.bank-payment.filter') }}" method="GET">
                        @method('GET')
                        <div class="row">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{route('erp.account-transaction.cash-bank-entry.bank-payment.index')}}" class="btn btn-danger" >Reset</a>
                </div>
            </div>
        </div>
    </div>
