            ...
            <div class="card">
                <div class="card-header p-3">
                    ...
                    <div class="col-auto ms-auto d-print-none pe-0">
                        <div class="btn-list">
                            <a href="{{ route('erp.account-transaction.cash-bank-entry.bank-receipt.create') }}" class="btn btn-sm btn-primary d-none d-sm-inline-block me-0">
                                <i class="fas fa-plus me-1"></i>
                                Add Bank Receipt
                            </a>

                            <a href="{{ route('erp.account-transaction.cash-bank-entry.bank-receipt.create') }}" class="btn btn-sm btn-primary d-sm-none me-0">
                ...
                <div class="card-body p-3">
                    <!-- <div class="table-responsive"> -->
                        <table class="table table-bordered text-nowrap border-bottom datatable-common">
                            ...
                            <tbody>
                                @foreach ($bankReceipts as $key => $bankReceipt)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ date('d-m-Y', strtotime($bankReceipt->date)) }}</td>
                                        <td>{{ $bankReceipt->account->name }}</td>
                                        <td>
                                            <label class="custom-control custom-checkbox">
                                            <input id="{{'is_audit_'.$bankReceipt->id}}" type="checkbox" onchange="updateIsAudit({{ $bankReceipt->id }})" class="custom-control-input" value="{{$bankReceipt->id}}" {{ $bankReceipt->is_audit == 1 ? 'checked' : ''}}>
                                                <span class="custom-control-label"></span>
                                            </label> 
                                        </td>
                                        <td>{{ $bankReceipt->reference_no }}</td>
                                        <td>{{ $bankReceipt->opposite_account->name }}</td>
                                        <td>{{ $bankReceipt->amount }}</td>
                                        <td>
                                            <div class="{{ $bankReceipt->is_audit == 1 ? '' : 'd-none'}}" id="{{'not-edited-'.$bankReceipt->id}}">
                                            ...
                                            <div class="{{ $bankReceipt->is_audit == 1 ? 'd-none' : ''}}" id="{{'edited-'.$bankReceipt->id}}">
                                                <a href="{{route('erp.account-transaction.cash-bank-entry.bank-receipt.edit', $bankReceipt->id)}}" class="btn btn-sm btn-outline-info">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </span>
                                                </a>
                                                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete-modal-{{ $bankReceipt->id }}">
                    ...
                    {{-- modals --}}
                    @foreach ($bankReceipts as $key => $bankReceipt)
                        <div class="modal fade" id="delete-modal-{{ $bankReceipt->id }}">
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.account-transaction.cash-bank-entry.bank-receipt.destroy', $bankReceipt->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('erp.account-transaction.cash-bank-entry.bank-receipt.filter') }}" method="GET">
                        @method('GET')
                        <div class="row">
                            ...
                            <div class="form-group col-md-6  mb-4">
                                <label for="type" class="form-label">Bank Account</label>
                                <input type="text" name="filter[account.name]" class="form-control form-control-sm">
                            </div>

                            <div class="form-group col-md-6  mb-4">
                                <label for="type" class="form-label">Cheque No</label>
                                <input type="text" name="filter[reference_no]" class="form-control form-control-sm">
                            </div>
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{route('erp.account-transaction.cash-bank-entry.bank-receipt.index')}}" class="btn btn-danger" >Reset</a>
                </div>
            </div>
        </div>
    </div>
