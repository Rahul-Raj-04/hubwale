            ...
            <div class="card">
                <div class="card-header p-3">
                    ...
                    <div class="col-auto ms-auto d-print-none pe-0">
                        <div class="btn-list">
                            <a href="{{ route('erp.account-transaction.cash-bank-entry.cash-receipt.create') }}" class="btn btn-sm btn-primary d-none d-sm-inline-block me-0">
                                <i class="fas fa-plus me-1"></i>
                                Add Cash Receipt
                            </a>

                            <a href="{{ route('erp.account-transaction.cash-bank-entry.cash-receipt.create') }}" class="btn btn-sm btn-primary d-sm-none me-0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <label class="custom-control custom-checkbox">
                                                <input id="{{'is_audit_'.$cashReceipt->id}}" type="checkbox" onchange="updateIsAudit({{ $cashReceipt->id }})" class="custom-control-input" value="{{$cashReceipt->id}}" {{ $cashReceipt->is_audit == 1 ? 'checked' : ''}}>
                                                <span class="custom-control-label"></span>
                                            </label> 
                                        </td>
                                        <td>{{ $cashReceipt->voucher_no ?? '-' }}</td>
                                        <td>{{ $cashReceipt->opposite_account->name }}</td>
                                        <td>{{ $cashReceipt->amount }}</td>
                                        <td>
                                            <div class="{{ $cashReceipt->is_audit == 1 ? '' : 'd-none'}}" id="{{'not-edited-'.$cashReceipt->id}}">
                                            ...
                                            <div class="{{ $cashReceipt->is_audit == 1 ? 'd-none' : ''}}" id="{{'edited-'.$cashReceipt->id}}">
                                                <a href="{{route('erp.account-transaction.cash-bank-entry.cash-receipt.edit', $cashReceipt->id)}}" class="btn btn-sm btn-outline-info">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </span>
                                                </a>
                            ...
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.account-transaction.cash-bank-entry.cash-receipt.destroy', $cashReceipt->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('erp.account-transaction.cash-bank-entry.cash-receipt.filter') }}" method="GET">
                        @method('GET')
                        <div class="row">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{route('erp.account-transaction.cash-bank-entry.cash-receipt.index')}}" class="btn btn-danger" >Reset</a>
                </div>
            </div>
        </div>
    </div>
