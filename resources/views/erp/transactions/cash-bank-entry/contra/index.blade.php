            ...
            <div class="card">
                <div class="card-header p-3">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Contra</li>
                    </ol>
                    <div class="col-auto ms-auto d-print-none pe-0">
                        <div class="btn-list">
                            <a href="{{ route('erp.account-transaction.cash-bank-entry.contra.create') }}" class="btn btn-sm btn-primary d-none d-sm-inline-block me-0">
                                <i class="fas fa-plus me-1"></i>
                                Add Contra
                            </a>

                            <a href="{{ route('erp.account-transaction.cash-bank-entry.contra.create') }}" class="btn btn-sm btn-primary d-sm-none me-0">
                ...
                <div class="card-body p-3">
                        ...
                        <table class="table table-bordered text-nowrap border-bottom datatable-common">
                            <thead>
                                <tr>
                                    ...
                                    <th class="bg-primary text-white">Cash/Bank A/c.</th>
                            ...
                            <tbody>
                                @foreach ($contras as $key => $contra)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{  date('d-m-Y', strtotime($contra->date)) }}</td>
                                        <td>{{ $contra->account->name }}</td>
                                        <td>
                                            <label class="custom-control custom-checkbox">
                                                <input id="{{'is_audit_'.$contra->id}}" type="checkbox" onchange="updateIsAudit({{ $contra->id }})" class="custom-control-input" value="{{$contra->id}}" {{ $contra->is_audit == 1 ? 'checked' : ''}}>
                                                <span class="custom-control-label"></span>
                                            </label> 
                                        </td>
                                        <td>{{ $contra->reference_no }}</td>
                                        <td>{{ $contra->opposite_account->name }}</td>
                                        <td>{{ $contra->amount }}</td>
                                        <td>
                                            <div class="{{ $contra->is_audit == 1 ? '' : 'd-none'}}" id="{{'not-edited-'.$contra->id}}">
                                            ...
                                            <div class="{{ $contra->is_audit == 1 ? 'd-none' : ''}}" id="{{'edited-'.$contra->id}}">
                                                <a href="{{route('erp.account-transaction.cash-bank-entry.contra.edit', $contra->id)}}" class="btn btn-sm btn-outline-info">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </span>
                                                </a>
                                                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete-modal-{{ $contra->id }}">
                    ...
                    {{-- modals --}}
                    @foreach ($contras as $key => $contra)
                        <div class="modal fade" id="delete-modal-{{ $contra->id }}">
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.account-transaction.cash-bank-entry.contra.destroy', $contra->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('erp.account-transaction.cash-bank-entry.contra.filter') }}" method="GET">
                        @method('GET')
                        <div class="row">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{route('erp.account-transaction.cash-bank-entry.contra.index')}}" class="btn btn-danger" >Reset</a>
                </div>
            </div>
        </div>
    </div>
