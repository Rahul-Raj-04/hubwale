            ...
            <div class="card">
                <div class="card-header p-3">
                    ...
                    <div class="col-auto ms-auto d-print-none d-flex pe-0">
                        ...
                        <div class="btn-list">
                            <label for="formSubmitBtn" class="btn btn-primary d-none btn-sm d-sm-inline-block me-0"><i class="fas fa-save me-1"></i> Save Service</label>
                            <label for="formSubmitBtn" class="btn btn-primary d-sm-none btn-sm btn-icon me-0"><i class="fas fa-plus mx-2"></i></label>
                        </div>                 
                    </div>
                </div>
                <div class="card-body p-3">
                    <form action="{{ route('erp.master.service.store') }}" method="POST">
                        @method('POST')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control form-control-sm" value="{{ old('alias') }}" placeholder="Enter alias"
                                    name="alias">
                                @error('alias')
                                    <span class="text-danger small wow flash">{{ $message }}</span>
                                @enderror
                                ...
                                <select name='gst_commodity_id' class="form-control form-control-sm form-select">
                                    ...
                                    @foreach ($gstCommoditys as $gstCommodity)
                                        <option value="{{ $gstCommodity->id }}"
                                            {{ old('gst_commodity_id') == $gstCommodity->id ? 'selected' : '' }}>{{ $gstCommodity->description }}
                                        </option>
                                    @endforeach
                                </select>
