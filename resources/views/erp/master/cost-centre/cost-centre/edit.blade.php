        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body p-3">
                    <div>
                        <form action="{{ route('erp.master.cost-centre.cost-centre.update', $costCentre->id) }}" method="POST">
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    <label class="form-label">Centre Name <i class="text-danger">*</i></label>
                                    <input type="text" class="form-control form-control-sm" value="{{ old('name') ? old('name') : $costCentre->name }}" placeholder="Enter Centre Name"
                                            name="name">
                                    @error('name')
                                ...
                                <div class="col-md-6 mb-4">
                                    ...
                                    <select name="cost_category_id" class="form-control form-control-sm form-select">
                                        ...
                                        @foreach ($costCategories as $costCategorie)
                                            <option value="{{ $costCategorie->id }}"  {{(old('cost_category_id') ? old('cost_category_id') : $costCentre->cost_category_id) == $costCategorie->id ? 'selected' : ''}}>
                                                {{ $costCategorie->name }}
                                            </option>
                                        @endforeach
                                    </select>
