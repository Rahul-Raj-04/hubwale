    ...
    <div class="page-header">
        ...
        <div>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Provisional Invoice</li>
            </ol>
        </div>
    </div>
    <!-- PAGE-HEADER END -->
                ...
                <div class="card-body">
                    <!-- <div class="table-responsive"> -->
                        <table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                            ...
                            <tbody>
                                @foreach ($provisionalInvoices as $key => $provisionalInvoice)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{  date('d-m-Y', strtotime($provisionalInvoice->bill_date)) }}</td>
                                        <td><input id="{{'is_audit_'.$provisionalInvoice->id}}" type="checkbox" onchange="updateIsAudit({{ $provisionalInvoice->id }})" value="{{$provisionalInvoice->id}}" {{ $provisionalInvoice->is_audit == 1 ? 'checked' : ''}}></td>
                                        <td>{{ $provisionalInvoice->account->balance_type }}</td>
                                        <td>{{ $provisionalInvoice->bill_no }}</td>
                                        <td>{{ $provisionalInvoice->account->name ?? '-' }}</td>
                                        <td>{{ $provisionalInvoice->account->city->name ?? '-' }}</td>
                                        <td>{{ $provisionalInvoice->amount }}</td>
                                        <td>
                                            <div class="{{ $provisionalInvoice->is_audit == 1 ? '' : 'd-none'}} not-edited" id="{{'not-edited-'.$provisionalInvoice->id}}">
                                            ...
                                            <div class="{{ $provisionalInvoice->is_audit == 1 ? 'd-none' : ''}}" id="{{'edited-'.$provisionalInvoice->id}}">
                                                <a href="{{ route('erp.consultant.provisional-invoice.edit', $provisionalInvoice->id) }}" class="btn btn-sm btn-outline-info">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </span>
                                                </a>
                                                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#delete-modal-{{ $provisionalInvoice->id }}">
                                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Delete">
                                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                                    </span>
                                                </button>
                    ...
                    {{-- modals --}}
                    @foreach ($provisionalInvoices as $key => $provisionalInvoice)
                        <div class="modal fade" id="delete-modal-{{ $provisionalInvoice->id }}">
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.consultant.provisional-invoice.destroy', $provisionalInvoice->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
...
@section('customJs')
    <script type="text/javascript">
        function updateIsAudit(id) {
            ...
            $.ajax({
                 type: "post",
                 url: "{{ route('erp.consultant.provisional-invoice.update-is-audit')}}",
