        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Create Bank Receipt</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('erp.consultant.bank-receipt.store') }}" method="POST">
                        @method('POST')
                        @csrf
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select name='account_id' class="form-control form-control-sm form-select" id="account_id" required>
                                ...
                                @foreach ($cashBankAccounts as $account)
                                    <option value="{{ $account->id }}" balance="{{ $account->ldger_account_balance ? $account->ldger_account_balance->balance : '' }}" parent-name="{{$account->account_group->name}}" {{ old('account_id') == $account->id ? 'selected' : '' }}>
                                    {{ $account->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @enderror
                            <label class="form-label">Balance : <span id="balance"></span></label>
                        </div>
                        <div class="col-md-6 mb-4">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select name='opposite_account_id' class="form-control form-control-sm form-select" id="opposite_account_id" required>
                                ...
                                @foreach ($oppAccounts as $account)
                                    <option value="{{ $account->id }}" balance="{{ $account->ldger_account_balance ? $account->ldger_account_balance->balance : '' }}" {{ old('opposite_account_id') == $account->id ? 'selected' : '' }}>
                                    {{ $account->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('opposite_account_id')
                                <span class="text-danger small wow flash">{{ $message }}</span>
                            @enderror
                            <label class="form-label"> Prov. Outstanding : <span id="opp_account_balance"></span></label>
                        </div>
                        <div class="col-md-6 mb-4">
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#account_id").change(function(){
                ...
                if (parent_account == 'Cash-in-hand'){
