            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="col-auto ms-auto d-print-none pe-0">
                        <div class="btn-list">
                            <a href="{{ route('erp.gst.gst-entry.cash-payment.create') }}" class="btn btn-sm btn-primary d-none d-sm-inline-block me-0">
                                <i class="fas fa-plus me-1"></i>
                                Add Cash Payment
                            </a>

                            <a href="{{ route('erp.gst.gst-entry.cash-payment.create') }}" class="btn btn-sm btn-primary d-sm-none me-0">
                            ...
                            <tbody>
                                @foreach ($cashPayments as $key => $cashPayment)
                                    <tr>
                                        ...
                                        <td>{{ $cashPayment->challan_no }}</td>
                                        <td>{{ $cashPayment->vou_no }}</td>
                                        <td>{{ $cashPayment->utilization_type }}</td>
                                        <td>{{ $cashPayment->amount }}</td>
                                        <td>
                                            <a href="{{route('erp.gst.gst-entry.cash-payment.edit', $cashPayment->id)}}" class="btn btn-sm btn-outline-info">Edit</a>
                            ...
                            <div class="modal-dialog modal-dialog-centered text-center" role="document">
                                <div class="modal-content tx-size-sm">
                                    <div class="modal-body text-center p-4 pb-5">
                                        ...
                                        <form action="{{route('erp.gst.gst-entry.cash-payment.destroy', $cashPayment->id)}}" method="POST" class="btn">
                                                @method('DELETE')
                                                @csrf
